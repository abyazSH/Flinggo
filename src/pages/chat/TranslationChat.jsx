...
export default function TranslationChat() {
  ...
  const { translateText, isLoading: isLlmLoading } = useLLM();
  
  // State manajemen riwayat obrolan murni untuk LLaMA
  ...
  async function handleSend(e) {
    ...
    try {
      // Pemrosesan Chatbot / Obrolan Interaktif
  ...
  const totalInteractionsCount = messages.filter(m => !m.isUser).length;
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          {messages.length === 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col items-center justify-center h-full text-center gap-4">
              ...
              <div>
                <p className="text-lg font-semibold">Ruang Obrolan LLaMA 3 Glossa Engine</p>
                <p className="text-sm text-base-content/50 max-w-sm mt-1">
                  Ketik teks dalam bahasa {srcLang.name} untuk mendapatkan keluaran respon translasi kustom atau berinteraksi langsung.
                </p>
              </div>
            ...
            <div className="w-[280px] p-4 flex flex-col h-full">
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-1.5">
                  <div className="badge badge-sm bg-blue-500/10 text-blue-500 border-blue-500/20">🦙 Llama 3 Glossa</div>
                </div>
              </div>

              <div className="flex-1 overflow-y-auto">
                <p className="text-[10px] uppercase tracking-wider text-base-content/40 font-semibold mb-2">Aktivitas Terakhir</p>
