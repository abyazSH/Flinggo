...
## Introduction
This document explains the AI model services powering the translation chat interface, focusing on the Gemma and Llama implementations. It covers service abstractions, API configuration, request formatting, response parsing, model-specific parameters, integration with the useLLM hook, error propagation, response normalization, model selection logic, fallback strategies, comparative performance metrics, API key management, and extension guidelines for adding additional AI models.

## Project Structure
The AI model services are organized around a small set of cohesive modules:
- Hook for unified AI interaction
- Provider-specific services for Gemma and Llama
- Translation orchestration service
- UI components for model selection and comparison
- Configuration for languages and Supabase integration
...
- translationService: Orchestrates model selection and comparison, normalizes responses, and compares outputs.
- gemmaService: Implements Google Generative AI integration with a system instruction and JSON-parsed response handling.
...
- UI layer handles user interactions and renders results.
- Hook layer encapsulates shared logic for loading and error states.
- Service layer abstracts provider-specific APIs and normalizes outputs.
- Configuration and persistence layer manages language metadata and storage.
